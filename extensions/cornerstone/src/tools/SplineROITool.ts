...
import type { Types } from '@cornerstonejs/core/dist/esm/types';
...
import { Events, MouseBindings, KeyboardBindings, ChangeTypes } from '@cornerstonejs/tools/src/enums';
import { resetElementCursor } from '@cornerstonejs/tools/src/cursors/elementCursor';
import type {
  Annotation,
  EventTypes,
  ToolHandle,
...
} from '@cornerstonejs/tools/src/utilities';
import getMouseModifierKey from '@cornerstonejs/tools/src/eventDispatchers/shared/getMouseModifier';
import { getViewportIdsWithToolToRender } from '@cornerstonejs/tools/src/utilities/viewportFilters';
import { getTextBoxCoordsCanvas } from '@cornerstonejs/tools/src/utilities/drawing';

import { ContourWindingDirection } from '@cornerstonejs/tools/src/types/ContourAnnotation';
import type { SplineROIAnnotation } from '@cornerstonejs/tools/src/types/ToolSpecificAnnotationTypes';
import type {
  AnnotationModifiedEventDetail,
  ContourAnnotationCompletedEventDetail,
} from '@cornerstonejs/tools/src/types/EventTypes';
import { ISpline } from '@cornerstonejs/tools/src/types/ISpline';
import { CardinalSpline } from '@cornerstonejs/tools/src/tools/annotation/splines/CardinalSpline';
import { LinearSpline } from '@cornerstonejs/tools/src/tools/annotation/splines/LinearSpline';
import { CatmullRomSpline } from '@cornerstonejs/tools/src/tools/annotation/splines/CatmullRomSpline';
import { BSpline } from '@cornerstonejs/tools/src/tools/annotation/splines/BSpline';
import ContourSegmentationBaseTool from '@cornerstonejs/tools/src/tools/base/ContourSegmentationBaseTool';
...
class SplineROITool extends ContourSegmentationBaseTool {
  ...
  addNewAnnotation(evt: EventTypes.InteractionEventType): SplineROIAnnotation {
    ...
    const contourHoleProcessingEnabled =
      getMouseModifierKey(evt.detail.event) === this.configuration.contourHoleAdditionModifierKey;
  ...
  _endCallback = (evt: EventTypes.InteractionEventType): void => {
    ...
    const { annotation, viewportIdsToRender, newAnnotation, contourHoleProcessingEnabled } =
      this.editData;
    const { data } = annotation;
    ...
    const changeType = newAnnotation ? ChangeTypes.Completed : ChangeTypes.HandlesUpdated;
  ...
  protected renderAnnotationInstance(renderContext: AnnotationRenderContext): boolean {
    ...
    const canvasCoordinates = controlPoints.map(p => worldToCanvas(p)) as Types.Point2[];
    ...
    const splineAnnotationsGroup = [annotation, ...childAnnotations].filter(annotation =>
  ...
  private _renderStats = (annotation, viewport, svgDrawingHelper, textboxStyle) => {
  ...
  private _updateSplineInstance(element: HTMLDivElement, annotation: SplineROIAnnotation): ISpline {
    const enabledElement = getEnabledElement(element);
    const { viewport } = enabledElement;
    const { worldToCanvas } = viewport;
    const { data } = annotation;
    ...
    const resolution =
      splineConfig.resolution !== undefined ? parseInt(splineConfig.resolution) : undefined;
    const scale = splineConfig.scale !== undefined ? parseFloat(splineConfig.scale) : undefined;

    spline.setControlPoints(canvasPoints);
    spline.closed = !!data.contour.closed;

    // Update spline resolution in case it has changed
    if (!spline.fixedResolution && resolution !== undefined && spline.resolution !== resolution) {
  ...
  private _calculateCachedStats = (annotation: SplineROIAnnotation, element: HTMLDivElement) => {
    ...
    for (let i = 0; i < targetIds.length; i++) {
      ...
      const { scale, areaUnits } = getCalibratedLengthUnitsAndScale(image, () => {
        ...
        const bottomRightBBWorld = viewport.canvasToWorld([canvasMaxX, canvasMaxY]);
